...
SimObject simObject=SimObjectPool[type].First.Value;SimObjectPool[type].RemoveFirst();simObject.DisabledNode=null;simObject.transform.rotation=Quaternion.Euler(rotation);simObject.transform.position=position;load_Syn_All.Add(simObject.load_Syn);if(!simObject.network.IsSpawned)simObject.network.Spawn();return simObject;
}else{
